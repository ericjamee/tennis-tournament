...
export async function register(_:FormState,formData:FormData):Promise<FormState>{
  ...
  const url=getSupabaseUrl(); const secretKey=getSupabaseSecretKey();
  if(!url||!secretKey)return{error:"Registration is temporarily unavailable. Please try again soon."};
  const {createClient}=await import("@supabase/supabase-js"); const db=createClient(url,secretKey);
  const {data:tournament,error:tournamentError}=await db.from("tournaments").select("id,name,date,venue_name,venue_address").eq("slug",TOURNAMENT_SLUG).single();
