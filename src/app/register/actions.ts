...
export async function register(_:FormState,formData:FormData):Promise<FormState>{
  ...
  const {data,error}=await db.rpc("register_for_tournament",{p_tournament_id:fallbackTournament.id,p_registration:{...parsed.data,rules_accepted:true,waiver_accepted:true}});
  ...
  const result=(Array.isArray(data)?data[0]:data) as {id:string;status:string};
  redirect(`/register/confirmation?id=${result.id}&name=${encodeURIComponent(`${parsed.data.first_name} ${parsed.data.last_name}`)}&status=${result.status}`);
}
